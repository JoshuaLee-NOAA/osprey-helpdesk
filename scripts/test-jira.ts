...
// Simple manual .env.local loader to load variables into process.env
...
async function runTest() {
  ...
  console.log(`- API_MODE: ${process.env.API_MODE || "LIVE (No Mock Mode active)"}\n`);

  const isMockMode = process.env.API_MODE === "MOCK";

  if (!isMockMode && (!process.env.JIRA_HOST || !process.env.JIRA_USER_EMAIL || !process.env.JIRA_API_TOKEN)) {
  ...
  if (isMockMode) {
    console.log("ℹ️ Note: Running in MOCK Mode. Real Jira credentials validation bypassed.\n");
  }

  try {
    // Dynamic import to avoid ES module hoisting trap and guarantee env is loaded first!
    const { searchJiraIssues, createJiraIssue } = await import("../src/lib/jira");

    console.log("1. Testing Issue Search...");
    const searchResults = await searchJiraIssues("MFA");
    console.log(`✅ Success! Found ${searchResults.length} existing tickets containing 'MFA'.`);
